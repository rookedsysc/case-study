...
interface CouponRedisLockV2ControllerDocs {
    @Operation(summary = "쿠폰 발행 v2", description = "Redis 락과 cache-aside 기반으로 쿠폰을 발행합니다.")
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = "201",
            ...
            ApiResponse(
                responseCode = "500",
                description = "쿠폰 락 해제 실패 또는 저장 중 무결성 오류",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))],
            ),
        ],
    )
