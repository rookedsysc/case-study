...
interface StoreControllerDocs {
    ...
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = "201",
                description = "생성 성공",
                content = [Content(schema = Schema(implementation = BulkCreateStoresResponse::class))],
            ),
            ApiResponse(
                responseCode = "400",
    ...
    fun createStoresBulk(
        @Parameter(description = "상점 일괄 생성 요청", required = true)
        request: BulkCreateStoresRequest,
