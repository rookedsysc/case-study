...
Redis 캐시 이후에도 충분한 QPS가 나오지 않았다. 여전히 데이터베이스 쿼리수는 300 ops/s 수준으로, 1 Core 1 GB의 인스턴스에서 감당하기엔 많은 양이었다. ops가 많은 이유는 사용자를 캐싱하고 있지만 최초로 로드할 때의 비용과 사용자에게 쿠폰을 발급할 때 저장 비용이 원인인 것 같다. 그래서 CQRS 패턴을 도입하면서 Replica DB를 2대를 두고 Event Streaming을 해서 데이터베이스에 부하를 줄이는 방식을 선택했다. 

Producing 할 때 유저가 쿠폰을 중복 발행했는지 검증을 할건지, Consuming을 할 때 검증을 할건지 고민을 했는데 아무래도 Consuming 하는 시점에 중복된 유저를 검증하는 방식은 중복 유저의 경우 다시 카운트를 올려줘야 하는데 그러면 모든 유저가 선착순으로 받을 수 없다는 제약조건에 위배되고 쓸데없는 리소스 사용량이 늘것 같았다. 
