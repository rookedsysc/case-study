...
- 각 유저별로 쿠폰 발급 여부를 로컬 캐시로 관리하기 위해서 Sticky Session 방식 도입
  - 도입 하지 않은 이유 : Sticky Session 방식은 특정 인스턴스에 트래픽이 몰리는 현상이 발생할 수 있고 애플리케이션 스케일 아웃이 힘들어지기 때문
- 유저별 중복 처리 : Bloom Filter 활용
  - 복잡성이 올라가 보류
  - 추후 구현 해볼 예정
