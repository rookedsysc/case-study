# 선착순 쿠폰 발급 시스템 성능 개선 기록

20,000명이 동시에 5,000장의 쿠폰을 선착순으로 요청하는 환경에서 `4,500 req/s` 처리량을 목표로 네 단계에 걸쳐 아키텍처를 개선했다.

| 단계 | 처리량 | 성공률 | 핵심 변경 |
|------|--------|--------|-----------|
| 비관적 락 | 148 req/s | 90.0% | DB 직렬화 기반 동시성 제어 |
| Redis 락 | 465 req/s | 96.6% | 경쟁 제어를 Redis로 이동, Cache Aside 적용 |
| Kafka 이벤트 스트리밍 | 1,923 req/s | 99.6% | DB write를 비동기화, CQRS 읽기/쓰기 분리 |
| 사전 검증(Pre-validation) | 1,566 req/s (822 it/s) | 100.0% | 적격성 사전 검증으로 불필요한 발급 차단, 도메인 책임 재배치 |

비관적 락에서 시작해 Redis 락으로 DB 병목을 완화하고, Kafka 이벤트 스트리밍으로 동기 쓰기를 제거한 뒤, 사전 검증(Pre-validation) 계층을 도입해 적격성 확인과 발급을 분리했다. 각 단계에서 발생한 정합성 문제(DB-Redis 트랜잭션 경계, 락 만료 경쟁 조건, 이벤트 발행 실패 보상)와 그 해결 과정을 함께 기록했다.
...
### 3. 병목 분석과 사전 검증(Pre-validation) 계층 도입
...
이 문제를 해결하기 위해 적격성 확인과 실제 발급을 분리하는 사전 검증(Pre-validation) 계층을 도입했다. 이 계층은 Fail-fast 전략으로 동작하여, 먼저 발행 가능한 쿠폰이 남아 있는지 확인하고 재고가 소진된 경우 즉시 응답을 반환한다. 재고가 남아 있는 경우에만 따닥 방지를 위한 TTL 기반 유저 중복 조회 구간을 거친 뒤 발급 요청을 전달한다. 이 단계에서 불필요한 발급 시도 자체를 사전에 차단함으로써 하위 시스템의 부하를 줄였다.
...
> 📊 [사전 검증 패턴 테스트 결과 보기](#사전-검증pre-validation-테스트-결과)
...
- `issue_coupon_success_rate`는 `90.03%`였고, `http_req_duration p(95)`는 `51.97s`, `issue_coupon_success_duration p(95)`는 `54.05s`로 응답 시간이 매우 길었다.
...
> 전체 요청 처리량이 약 `147.58 req/s`에 머물고 `http_req_duration p(95)`가 `51.97s`까지 올라가면서, 목표 QPS `4500 req/s`에 크게 못 미친 것을 보여준다.
...
Kafka 이벤트 스트리밍으로 전환하자 요청 처리 경로에서 가장 비싼 DB 저장 작업이 빠지면서 처리량이 Redis 락 대비 다시 `4배` 이상 늘었고, 성공률도 `99.59%`까지 올라갔다. 여전히 목표 QPS `4500 req/s`에는 못 미쳤지만, 병목의 중심이 DB에서 애플리케이션/Redis/Kafka 조합으로 이동하면서 전체 시스템이 훨씬 높은 부하를 흡수할 수 있게 됐다. 즉 이 시나리오에서는 동기 DB write를 직접 줄이는 비동기 이벤트 스트리밍 전략이 가장 큰 성능 개선 효과를 보였다.

## 사전 검증(Pre-validation) 테스트 결과

사전 검증 시나리오에서는 Kafka 이벤트 스트리밍 대비 성공률과 안정성이 한층 더 개선됐다. 적격성 확인과 쿠폰 발급을 별도 요청으로 분리하면서 불필요한 발급 시도 자체를 줄였고, 전체 실패율이 `0%`까지 내려갔다.

### 결과 요약

- k6 기준 전체 HTTP 요청 처리량은 약 `1566 req/s`였고, iteration 처리량은 약 `822 it/s`였다. 사전 검증 패턴은 적격성 확인과 쿠폰 발급을 두 번의 요청으로 나누기 때문에 iteration당 약 2회의 HTTP 요청이 발생한다.
...
![사전 검증 k6 결과](./img/pre-validation-k6-result.png)
...
![사전 검증 Redis 사용량 지표](./img/pre-validation-redis.png)
...
사전 검증 계층을 도입하자 적격성 확인과 쿠폰 발급이 분리되면서, 재고가 소진된 이후의 불필요한 발급 요청이 사전에 차단됐다. 그 결과 `issue_coupon_success_rate`가 `100%`, 전체 실패율이 `0%`로 이전 시나리오 대비 안정성이 가장 높았다. HTTP 요청 처리량 자체는 Kafka 시나리오의 `1922 req/s` 대비 다소 낮은 `1566 req/s`였지만, 이는 iteration당 2회 요청이 발생하는 구조적 특성에 기인한다. iteration 기준으로는 `822 it/s`로 실질적인 사용자 처리량이 충분히 유지됐고, 응답 시간 분포도 Kafka 시나리오와 유사한 수준이었다. DB와 Redis 부하도 안정적으로 유지되어, 사전 검증 패턴이 성능을 유지하면서도 시스템 안정성을 한 단계 끌어올린 것으로 확인됐다.
...
### 사전 검증(Pre-validation) 방식

```mermaid
sequenceDiagram
    participant Client
    participant Gateway as Pre-validation (Application)
    participant Redis
    participant Kafka
    participant Consumer
    participant DB as Source DB

    Client->>Gateway: GET /api/v5/coupons/kafka/eligibility

    Gateway->>Gateway: isSoldOut (로컬 캐시)
    alt 솔드아웃
        Gateway-->>Client: eligible=false (SOLD_OUT)
    end

    Gateway->>Redis: isCouponIssued (SISMEMBER)
    alt 이미 발급됨
        Gateway-->>Client: eligible=false (ALREADY_ISSUED)
    end

    Gateway->>Redis: isDedupKeyPresent (따닥 방지)
    alt 처리 중
        Gateway-->>Client: eligible=false (IN_PROGRESS)
    else 적격
        Gateway-->>Client: eligible=true
    end

    Client->>Gateway: POST /api/v5/coupons/kafka

    Gateway->>Gateway: isSoldOut (로컬 캐시)
    Gateway->>Redis: isCouponIssued (SISMEMBER)
    Gateway->>Redis: tryAcquireDedup (TTL 키 설정)

    Gateway->>Kafka: publishAsync(event)
    ...
    alt 재고 없음
        ...
        Note over Consumer: 이후 사전 검증 계층에서 즉시 차단
    else 재고 차감 성공
