...
| BFF 패턴 | 1,566 req/s (822 it/s) | 100.0% | 적격성 확인과 발급 분리, 도메인 책임 재배치 |

비관적 락에서 시작해 Redis 락으로 DB 병목을 완화하고, Kafka 이벤트 스트리밍으로 동기 쓰기를 제거한 뒤, BFF 패턴으로 적격성 확인과 발급을 분리했다. 각 단계에서 발생한 정합성 문제(DB-Redis 트랜잭션 경계, 락 만료 경쟁 조건, 이벤트 발행 실패 보상)와 그 해결 과정을 함께 기록했다.
